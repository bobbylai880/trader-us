#!/usr/bin/env python3
"""
V7.2 "LLM Theme" 交易系统回测 - LLM生成主题版

对比 V7.0 (人工主题) vs V7.1 (规则主题) vs V7.2 (LLM主题):
- V7.0: 人工基于历史事件判断每季度主题
- V7.1: 系统基于历史事件自动生成主题 (规则模式)
- V7.2: Oracle Agent (Claude) 基于历史事件深度分析生成主题
...
def load_quarterly_themes() -> Dict:
    if LLM_THEMES_FILE.exists():
        with open(LLM_THEMES_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    return {
        "default": {
            "theme": "均衡配置",
...
def main():
    bt = V72BacktestEngine(100000.0)
    result = bt.run(date(2023, 1, 3), date(2026, 1, 16))
    
    print("\n" + "=" * 70)
    print("V7.2 回测结果 (3年) - LLM主题版")
    print("=" * 70)
    ...
    output = Path("storage/backtest_v7_2")
    ...
    print("\n" + "=" * 70)
    print("【V7.0 vs V7.1 vs V7.2 策略对比】")
    print("=" * 70)
    
    v70_result_file = Path("storage/backtest_v7_0/result.json")
    v71_result_file = Path("storage/backtest_v7_1/result.json")
    ...
    if v70_result_file.exists():
        ...
        v71 = None
        if v71_result_file.exists():
            with open(v71_result_file) as f:
                v71 = json.load(f)
        
        print(f"\n  {'指标':<12} {'V7.0(人工)':<15} {'V7.1(规则)':<15} {'V7.2(LLM)':<15}")
        print(f"  {'-'*57}")
        v71_ret = f"{v71['total_return']:+.1%}" if v71 else "N/A"
        v71_alpha = f"{v71['alpha']:+.1%}" if v71 else "N/A"
        v71_dd = f"{v71['max_drawdown']:.1%}" if v71 else "N/A"
        v71_sharpe = f"{v71['sharpe']:.2f}" if v71 else "N/A"
        print(f"  {'总收益':<12} {v70['total_return']:+.1%:<15} {v71_ret:<15} {result['total_return']:+.1%:<15}")
        print(f"  {'Alpha':<12} {v70['alpha']:+.1%:<15} {v71_alpha:<15} {result['alpha']:+.1%:<15}")
        print(f"  {'回撤':<12} {v70['max_drawdown']:.1%:<15} {v71_dd:<15} {result['max_drawdown']:.1%:<15}")
        print(f"  {'夏普':<12} {v70['sharpe']:.2f:<15} {v71_sharpe:<15} {result['sharpe']:.2f:<15}")
        
        print("\n  📊 结论:")
        alpha_diff_v70 = (result['alpha'] - v70['alpha']) * 100
        if alpha_diff_v70 > 1:
            print(f"     ✅ LLM主题优于人工主题 (Alpha +{alpha_diff_v70:.1f}pp)")
        elif alpha_diff_v70 < -1:
            print(f"     ❌ LLM主题不如人工主题 (Alpha {alpha_diff_v70:.1f}pp)")
        else:
            print(f"     ➖ LLM主题与人工主题相当 (Alpha {alpha_diff_v70:+.1f}pp)")
        
        if v71:
            alpha_diff_v71 = (result['alpha'] - v71['alpha']) * 100
            if alpha_diff_v71 > 1:
                print(f"     ✅ LLM主题优于规则主题 (Alpha +{alpha_diff_v71:.1f}pp)")
    else:
